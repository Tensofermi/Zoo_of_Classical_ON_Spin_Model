...
path = f"fit_data/fitting_{format(sigma, '.3f')}"

# data = ld.load_data(path, index_x, index_y)

#  L,     K,      O,     O_err 
data_x, data_y, data_z, data_zerr = ld.load_data_xyz(path, index_x, index_y, index_z)

L_min_list = [32,64,128,256,512]
######################################
##### Fitting Function ###############
######################################
...
def create_params() -> lt.Parameters:
    ...
    params.add('Kc', value=0.312, vary=True, min=0.25)
    params.add('yt', value=1.0, vary=True)
    
    params.add('a0', value=-0.1, vary=True)
    params.add('a1', value=0.1, vary=True)
    params.add('a2', value=0.1, vary=True)
    params.add('a3', value=0, vary=False)
    params.add('a4', value=0, vary=False)

    params.add('y1', value=-1.0, vary=False, max = 0.0)
    params.add('b1', value=0.1, vary=True)

    params.add('b2', value=0, vary=False)
    params.add('b3', value=0, vary=False)
...
for L_min in L_min_list: 
    ...
    fit_y = data_y[i:]
    fit_z = data_z[i:]
    fit_zerr = 1*data_zerr[i:]

    model = lt.Model(ssb_model, independent_vars=['L', 'K'])
    params = create_params()

    result = model.fit(fit_z, params, L = fit_x, K = fit_y, weights = 1 / fit_zerr)
