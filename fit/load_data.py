...
def load_data_xyz(file_path: str, index_col_x: int, index_col_y: int, index_col_z: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    从文件中加载数据
    ...
    Returns:
        np.ndarray: 加载的数据数组
    """
    data = np.loadtxt(file_path, usecols=(index_col_x, index_col_y, index_col_z, index_col_z + 1), comments='#')
    x = data[:, 0]
    y = data[:, 1]
    z = data[:, 2]
    z_err = data[:, 3]
