...
void BasicObservable::calBlock(unsigned long _NBin, unsigned long _NperBin)
{
	...
	if (n == 0 || _NperBin == 0)
		throw std::invalid_argument("insufficient samples for block averaging");
...
std::string BasicObservable::printCurrent(int Index)
{
	char a[150];
    std::string str;
	    std::snprintf(a, sizeof(a), "%-4d %-12.12s = % -16.8e  %-40.40s\n",Index,Name.c_str(),A+B*Value,Des.c_str());
    str = a;
    return  str;
}
